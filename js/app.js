...
window.exportDataToCSV = (tipo, filename) => {
    ...
    if(dataToExport.length === 0) { 
        alert("Sem dados processados para exportar."); 
        return; 
    }
...
// ==========================================
// 4. MOTOR DE INTEGRAÇÃO (GOOGLE SHEETS)
// ==========================================
const submitToSheets = async (form, btnId, msgSuccessId, msgErrorId, payload, btnOriginalText) => {
    ...
    if(btn) { 
        btn.disabled = true; 
        btn.innerHTML = '<i data-lucide="loader-2" class="w-5 h-5 animate-spin"></i> A enviar...'; 
    }
    ...
    try {
        ...
        if (result.status === 'success') {
            // Se for atualização de validade (Auditoria de Risco)
            ...
            // Recarrega os dados em background para atualizar painéis
...
window.renderQuebrasDashboard = () => {
    ...
    if(filtroFilial && filtroFilial !== 'todas') {
        dadosMes = dadosMes.filter(i => String(i.filial).trim() === String(filtroFilial).trim());
    }
    ...
    dadosMes.forEach(item => { 
        ...
        const custo = parseLocalFloat(item.custo); 
        const valorTotal = qtd * custo; 
        const motivo = item.motivo || 'Outros'; 
        const produto = item.descricao || 'Produto sem nome';
        
        totalRs += valorTotal; 
        totalQtd += qtd; 
        
        if(!motivosMap[motivo]) motivosMap[motivo] = 0; 
        motivosMap[motivo] += valorTotal; 
        
        if(!rankingMap[produto]) rankingMap[produto] = 0; 
        rankingMap[produto] += valorTotal;
    });

    if(document.getElementById('ui-quebra-total-rs')) {
        document.getElementById('ui-quebra-total-rs').innerText = 'R$ ' + totalRs.toLocaleString('pt-BR', {minimumFractionDigits: 2});
    }
    if(document.getElementById('ui-quebra-total-qtd')) {
    ...
    if(divChart && typeof ApexCharts !== 'undefined') {
        ...
        const options = {
            series: Object.values(motivosMap), 
        ...
        dadosDocas.slice(-5).reverse().forEach(item => { 
            divLista.innerHTML += `
                <div class="p-3 border-b border-slate-100 last:border-0">
                    <p class="font-bold text-navy">${item.fornecedor || 'Fornecedor'}</p>
                    <p class="text-sm text-slate-600">NF: ${item.nf || 'S/N'} | Divergência: <span class="font-bold text-red-600">${item.quantidade} un</span></p>
                </div>`;
        });
    }
};
...
window.renderValidadeDashboard = () => {
    ...
    if(filtroFilial && filtroFilial !== 'todas') {
        dadosValidade = dadosValidade.filter(i => String(i.filial).trim() === String(filtroFilial).trim());
    }
...
window.renderFurtosDashboard = () => {
    ...
    if(divChartPerfil && typeof ApexCharts !== 'undefined') {
        ...
        chartFurtosPerfilInstance = new ApexCharts(divChartPerfil, { 
    ...
    if(divChartLocais && typeof ApexCharts !== 'undefined') {
        ...
        chartFurtosLocaisInstance = new ApexCharts(divChartLocais, { 
...
window.renderPrecoDashboard = () => {
    ...
    dadosPreco.forEach(item => { 
        if (item.sem_preco === 'SIM') semPreco++; 
        else if (parseLocalFloat(item.preco_sistema) !== parseLocalFloat(item.preco_gondola)) divergentes++; 
    });
...
window.renderCaixaDashboard = () => {
    ...
    let totalRs = 0; 
    dadosCaixa.forEach(item => { totalRs += parseLocalFloat(item.valor_falta); });
...
window.renderTarefasDashboard = () => {
    ...
    // Alertas do Sistema (Baseado na aba de Validade)
    ...
    validades.forEach(v => {
        ...
        if(dias <= 15 && dias >= 0) {
            ...
            htmlSis += `
            <div class="p-3 bg-red-50 border border-red-200 rounded-lg flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
                <div class="flex items-start gap-3">
                    <i class="w-5 h-5 text-red-600 mt-1" data-lucide="alert-triangle"></i>
                    <div>
                        <p class="text-sm font-bold text-red-800">Risco: ${v.descricao || 'Produto sem nome'}</p>
                        <p class="text-xs text-red-600 font-medium">Vence em ${dias} dias | Filial: ${v.filial}</p>
                    </div>
                </div>
                <button onclick="abrirModalAuditoria('${itemEnc}')" class="w-full md:w-auto bg-red-600 hover:bg-red-700 text-white text-xs font-bold px-4 py-2 rounded shadow-sm transition-colors">
                    Auditar
                </button>
            </div>`;
        }
    ...
    tarefas.forEach(t => { 
        ...
        htmlMan += `
        <div class="p-3 bg-slate-50 border border-slate-200 rounded-lg flex items-center justify-between transition-opacity duration-300">
            <div class="flex items-center gap-3">
                <input type="checkbox" onchange="window.concluirTarefa('${tituloEnc}', '${t.filial}', this)" class="w-5 h-5 rounded border-slate-300 text-navy focus:ring-navy cursor-pointer">
                <div>
                    <p class="text-sm font-bold text-navy">${t.titulo}</p>
                    <p class="text-xs text-slate-500 font-medium">Prazo: ${t.prazo} | Filial: ${t.filial}</p>
                </div>
            </div>
        </div>`; 
    });

    divSis.innerHTML = htmlSis || '<p class="text-sm text-slate-400 text-center py-4">Nenhum risco sistêmico detectado.</p>';
    divMan.innerHTML = htmlMan || '<p class="text-sm text-slate-400 text-center py-4">Nenhuma demanda pendente.</p>';
    if(window.lucide) lucide.createIcons();
};
...
window.concluirTarefa = async (tituloEncoded, filial, checkboxEl) => {
    ...
    try {
        ...
        if(idx > -1) sheetsDataRaw[idx].status = 'CONCLUÍDA'; 
        window.renderTarefasDashboard(); 
    } catch(e) { 
...
window.renderListaInventarios = () => {
    ...
    listaArr.forEach(inv => {
        ...
        <tr class="hover:bg-slate-50 transition-colors border-b border-slate-100">
            ...
            <td class="px-6 py-4 text-right space-x-2">${btnContinuar} ${btnExportar}</td>
        </tr>`;
    });
...
window.iniciarNovoInventario = (event) => {
    ...
    if(document.getElementById('inv-lista-gtins')) document.getElementById('inv-lista-gtins').value = '';
    if(document.getElementById('inv-is-dirigido')) document.getElementById('inv-is-dirigido').checked = false;
    if(document.getElementById('inv-box-dirigido')) document.getElementById('inv-box-dirigido').classList.add('hidden');
...
window.encerrarInventarioAtual = async (event) => {
    ...
    if(!confirm(`⚠️ ATENÇÃO: Deseja encerrar o inventário ${idInv}? Produtos dirigidos não bipados serão negativados no sistema.`)) return;
    ...
    try {
        ...
        alert('Erro ao encerrar inventário.'); 
    } finally { 
...
document.getElementById('form-inventario')?.addEventListener('submit', async (e) => {
    ...
    const gtin = inputGtin.value; 
    const desc = document.getElementById('inv-desc')?.value || ""; 
    const qtd = inputQtd.value;
    
    const payload = { 
        tipo: "inventario", email: auth.currentUser.email, empresa: currentUserEmpresa, filial: filial, 
        lote: lote, gtin: gtin, descricao: desc, quantidade: qtd, id_inventario: idInv, status: "ABERTO" 
    };
    
    await submitToSheets(null, 'btn-save-inv', '', '', payload, '<i data-lucide="plus-square" class="w-5 h-5 text-gold"></i> Salvar Bipagem');
    
    sheetsDataRaw.push({ tipo: "inventario", lote: lote, gtin: gtin, descricao: desc, quantidade: qtd, id_inventario: idInv, status: "ABERTO", filial: filial });
    
    inputGtin.value = ''; 
    document.getElementById('inv-desc').value = '';
    
    const toggleQtd = document.getElementById('inv-travar-qtd');
    if(toggleQtd && !toggleQtd.checked) inputQtd.value = '';
...
window.renderHistoricoBipagem = (idInv) => {
    ...
    } else if (boxProgresso) { 
        boxProgresso.classList.add('hidden'); 
    }
...
document.getElementById('form-quebras')?.addEventListener('submit', (e) => {
    ...
    const payload = { 
        tipo: "quebra", email: auth.currentUser.email, empresa: currentUserEmpresa, filial: fil, mes: mF, 
        gtin: document.getElementById('q-gtin')?.value||"", descricao: document.getElementById('q-desc')?.value||"", 
        quantidade: document.getElementById('q-qtd')?.value||"", custo: document.getElementById('q-custo')?.value||"", motivo: document.getElementById('q-motivo')?.value||"" 
    };
...
document.getElementById('form-recebimento')?.addEventListener('submit', (e) => {
    e.preventDefault(); if (!auth.currentUser) return; 
    const fil = document.getElementById('r-filial-lancamento')?.value || currentUserFilial;
    
    const payload = { 
        tipo: "recebimento", email: auth.currentUser.email, empresa: currentUserEmpresa, filial: fil, 
        data_entrega: document.getElementById('r-data')?.value||"", fornecedor: document.getElementById('r-fornecedor')?.value||"", 
        nf: document.getElementById('r-nf')?.value||"", descricao: document.getElementById('r-desc')?.value||"", 
        quantidade: document.getElementById('r-qtd')?.value||"", custo: document.getElementById('r-custo')?.value||"", 
        motivo: document.getElementById('r-motivo')?.value||"", observacoes: document.getElementById('r-obs')?.value||"" 
    };
    submitToSheets(e.target, 'btn-save-recebimento', 'msg-rec-success', 'msg-rec-error', payload, 'Enviar Registo de Docas');
});

document.getElementById('form-validade')?.addEventListener('submit', (e) => {
    e.preventDefault(); if (!auth.currentUser) return; 
    const fil = document.getElementById('v-filial-lancamento')?.value || currentUserFilial;
    
    const payload = { 
        tipo: "validade", email: auth.currentUser.email, empresa: currentUserEmpresa, filial: fil, 
        gtin: document.getElementById('v-gtin')?.value||"", descricao: document.getElementById('v-desc')?.value||"", 
        categoria: document.getElementById('v-cat')?.value||"", quantidade: document.getElementById('v-qtd')?.value||"", 
        custo: document.getElementById('v-custo')?.value||"", data_validade: document.getElementById('v-data')?.value||"" 
    };
...
document.getElementById('form-auditoria-preco')?.addEventListener('submit', async (e) => {
    e.preventDefault(); if (!auth.currentUser) return; 
    const fil = document.getElementById('p-filial-lancamento')?.value || currentUserFilial;
    const inpD = document.getElementById('p-data'); const dS = inpD ? inpD.value : "";
    
    const payload = { 
        tipo: "auditoria_preco", email: auth.currentUser.email, empresa: currentUserEmpresa, filial: fil, 
        data_auditoria: dS, gtin: document.getElementById('p-gtin')?.value||"", descricao: document.getElementById('p-desc')?.value||"", 
        preco_sistema: document.getElementById('p-sistema')?.value||"", preco_gondola: document.getElementById('p-gondola')?.value||"", 
    ...
    if(inpD) inpD.value = dS; 
    const inG = document.getElementById('p-gtin'); if(inG) setTimeout(() => inG.focus(), 100);
});

document.getElementById('form-caixa-central')?.addEventListener('submit', (e) => {
    e.preventDefault(); if (!auth.currentUser) return; 
    const fil = document.getElementById('c-filial-lancamento')?.value || currentUserFilial;
    
    const payload = { 
        tipo: "caixa_central", email: auth.currentUser.email, empresa: currentUserEmpresa, filial: fil, 
        data_auditoria: document.getElementById('c-data')?.value||"", operador: document.getElementById('c-operador')?.value||"", 
        tipo_divergencia: document.getElementById('c-tipo')?.value||"", valor_falta: document.getElementById('c-valor')?.value||"", 
        observacoes: document.getElementById('c-obs')?.value||"" 
    };
    submitToSheets(e.target, 'btn-save-caixa', 'msg-caixa-success', 'msg-caixa-error', payload, 'Registrar Falta no Cofre');
});

document.getElementById('btn-add-prod')?.addEventListener('click', () => {
    const n = document.getElementById('f-prod-nome').value.trim(); 
    const q = parseInt(document.getElementById('f-prod-qtd').value)||0; 
    const p = parseFloat(document.getElementById('f-prod-preco').value.replace(',','.'))||0;
    
    if(!n || q <= 0 || p < 0) { alert("Preencha dados válidos do produto."); return; }
    
    produtosFurto.push({ nome: n, qtd: q, preco: p }); 
    document.getElementById('f-prod-nome').value = ''; document.getElementById('f-prod-qtd').value = ''; document.getElementById('f-prod-preco').value = '';
    
    const lP = document.getElementById('f-lista-produtos'); 
    if(lP) lP.innerHTML += `<li class="flex justify-between items-center p-2 bg-white border border-slate-200 rounded mb-1 text-xs shadow-sm"><span class="font-bold text-navy truncate flex-1">${n}</span><span class="text-slate-500 w-16 text-center">${q} un</span><span class="text-red-600 font-bold w-24 text-right">R$ ${(q*p).toLocaleString('pt-BR', {minimumFractionDigits: 2})}</span></li>`;
});

document.getElementById('form-furtos')?.addEventListener('submit', async (e) => {
    e.preventDefault(); if (!auth.currentUser) return;
    if (produtosFurto.length === 0) { alert("Adicione pelo menos um produto na lista de recuperados."); return; }
    
    const fil = document.getElementById('f-filial')?.value || currentUserFilial;
    
    const payload = { 
        tipo: "furto", email: auth.currentUser.email, empresa: currentUserEmpresa, filial: fil, 
        data_ocorrencia: document.getElementById('f-data')?.value||"", genero: document.getElementById('f-genero')?.value||"", 
        idade: document.getElementById('f-idade')?.value||"", abordagem: document.getElementById('f-abordagem')?.value||"", 
        local: document.getElementById('f-local')?.value||"", descricao: document.getElementById('f-desc')?.value||"", 
...
document.getElementById('form-auditoria')?.addEventListener('submit', async (e) => {
    ...
    const payload = { 
        tipo: "atualizar_validade", email: auth.currentUser.email, empresa: currentUserEmpresa, filial: currentUserFilial, 
        gtin: itemEmAuditoria.gtin, descricao: itemEmAuditoria.descricao, data_validade: itemEmAuditoria.data_validade, 
...
window.mudarEstadoSegmento = (est) => {
    ...
    if(vc) { vc.classList.remove('estado-hub', 'estado-varejo', 'estado-industria'); vc.classList.add('estado-' + est); }
...
if(lF) { 
    lF.addEventListener('submit', async (e) => { 
        ...
        try { 
            ...
            document.getElementById('login-error-text').innerText = "Credenciais inválidas ou acesso não autorizado."; 
        } 
    }); 
}
...
window.loadEmpresasAdmin = async () => {
    ...
    try { 
        ...
        let opt = '<option value="">Selecione...</option>'; const emp = []; 
        snap.forEach(d => emp.push(d.data().nome)); 
        emp.sort().forEach(e => opt += `<option value="${e}">${e}</option>`); 
        sel.innerHTML = opt; 
    } catch(e) {}
};
        ...
        if (user.email === 'leandro@lucroseguro.com.br' || user.email.includes('leandro')) { 
            ...
            try {
                ...
                if (docSnap.exists()) {
                    ...
                    alert("Acesso Negado: Usuário sem filial vinculada."); 
                    signOut(auth); 
                }
            } catch(e) { 
                alert("Erro de hierarquia."); 
                signOut(auth); 
            }
        }
    } else { 
